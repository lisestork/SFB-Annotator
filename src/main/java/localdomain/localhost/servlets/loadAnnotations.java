...
public class loadAnnotations extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
    		...
    		String query = "SELECT url, x, y, height, width, class_model, organismID FROM Annotations WHERE firstname = '"+firstname+"' and lastname='"+lastname+"'";
    		String countQuery = "SELECT count(*) FROM (SELECT url, x, y, height, width, class_model, organismID FROM Annotations WHERE firstname = '"+firstname+"' and lastname='"+lastname+") class_model";
            ...
            if (nr != 0) {
                ...
                while (result.next()) {		
        			anno = anno + result.getString("url")+","+result.getString("class_model")+","+result.getString("x")+","+result.getString("y")+","+result.getString("width")+","+result.getString("height")+","+result.getString("organismID")+"\n";
        		}
        		anno.toString();           	
            } else {
            	anno = "no annotations";
