	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/INSERT_DATABASE_NAME", "INSERT_USERNAME",
					"INSERT_PASSWORD");
			String insert = "INSERT INTO users (firstname, lastname, institution) SELECT '" + firstname + "','"
					+ lastname + "','" + institution
					+ "' FROM dual WHERE NOT EXISTS (SELECT 1 FROM users WHERE firstname = '" + firstname
					+ "' AND lastname = '" + lastname + "' AND institution = '" + institution + "');";
